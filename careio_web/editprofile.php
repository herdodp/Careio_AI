...
if(isset($_POST['submit'])){
    ...
    $fullnametake = $_POST['fullname'];
    $categorytake = $_POST['category'];
    $usernametake = $_POST['username'];


    //sanitize 1
    $fullnametake1 = mysqli_real_escape_string($koneksi, $fullnametake);
    $categorytake1 = mysqli_real_escape_string($koneksi, $categorytake);
    $usernametake1 = mysqli_real_escape_string($koneksi, $usernametake);

    //sanitize 2
    $fullnametake2 = strip_tags($fullnametake1);
    $categorytake2 = strip_tags($categorytake1);
    $usernametake2 = strip_tags($usernametake1);

    //cek username
    $takedata = mysqli_query($koneksi, "SELECT * FROM user where username = '$usernametake2'");
    if(mysqli_num_rows($takedata) > 0){
        ?>
        <script>
            setTimeout(function(){
                alert("Data tidak ditemukan. Login dengan akun yang sudah terdaftar.");
        ...
        $insertdata = mysqli_query($koneksi, "UPDATE user SET fullname = '$fullnametake2', category = '$categorytake2', username = '$usernametake2' WHERE username = '$usernamesession'");
        if($insertdata){
            ?>
            <script>
                setTimeout(function(){
                    alert("Berhasil mengubah data. Berpindah ke halaman profil dalam 2 detik");
                    window.location.href = "profil.php?i=<?php echo $usernametake2; ?>";
                }, 1000);
            </script>
            <?php  
        }
...
<body>
    ...
    if(mysqli_num_rows($datauser) > 0){
        while($data = mysqli_fetch_array($datauser)){
            $fullnamedb = $data['fullname'];
            $categorydb = $data['category'];
            $pindb = $data['pin'];
            $timeregistdb = $data['time_regist']; 
        }
    }else{
        ?>
        <script>
        ...
        <div class="profile-card">
            ...
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <div class="info-value"><input type="text" name="username" value="<?php echo $fullnamedb; ?>"></div>
                </div>

                 <div class="info-item">
                    <div class="info-label">Username</div>
                    <div class="info-value"><input type="text" name="username" value="<?php echo $usernamesession; ?>"></div>
                </div>

                 <div class="info-item">
                    <div class="info-label">Category</div>
                    <div class="info-value"><input type="text" name="username" value="<?php echo $categorydb; ?>"></div>
                </div>
            ...
            <div class="button-group">
                <input type="submit" name="submit" value="Change" class="btn-edit">
            </div>
