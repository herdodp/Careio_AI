...
<body>
    ...
    <div class="login-container">
        <div class="logo">
            <h1>Login</h1>
            <p>Login to access AI diagnostic tools</p>
        </div>
        ...
        if(isset($_POST['submit'])){
            ...
            $takedata = mysqli_query($koneksi, "SELECT * FROM user WHERE username = '$username1'");
            if(mysqli_num_rows($takedata) > 0){
                while($datauser = mysqli_fetch_array($takedata)){
                    $usernamedb = $datauser['username'];
                    $passworddb = $datauser['password'];
                }

                if($username1 == $usernamedb){
                    if($password1 == $passworddb){
